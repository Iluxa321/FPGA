...
output [WIDTH_DATA-1:0] amp
	...
	signed_mult #(.WIDTH_A(WIDTH_DATA+1), .WIDTH_B(WIDTH_SIN_COS)) mult_adc_const(.a({1'b0,reg_data}), .b(9'd255), .m(correct_adc));
	registor #(.WIDTH(WIDTH_1)) reg_adc(.clk(clk), .en(reg_en), .d(correct_adc - dc_amp), .q(correct_adc_reg));
	//---------------------------------------------
	
	wire [WIDTH_SIN_COS-1:0] reg2_sin, reg2_cos; // для симметрии
	registor #(.WIDTH(WIDTH_SIN_COS)) r_sin(.clk(clk), .en(reg_en), .d(reg_sin), .q(reg2_sin));
	registor #(.WIDTH(WIDTH_SIN_COS)) r_cos(.clk(clk), .en(reg_en), .d(reg_cos), .q(reg2_cos));
	...
	signed_mult #(.WIDTH_A(WIDTH_SIN_COS), .WIDTH_B(WIDTH_SIN_COS)) mult_sin_sin(.a(reg2_sin), .b(reg2_sin), .m(sin2));
	signed_mult #(.WIDTH_A(WIDTH_SIN_COS), .WIDTH_B(WIDTH_SIN_COS)) mult_cos_cos(.a(reg2_cos), .b(reg2_cos), .m(cos2));
	signed_mult #(.WIDTH_A(WIDTH_SIN_COS), .WIDTH_B(WIDTH_SIN_COS)) mult_sin_cos(.a(reg2_sin), .b(reg2_cos), .m(sin_cos));
	signed_mult #(.WIDTH_A(WIDTH_1), .WIDTH_B(WIDTH_SIN_COS)) mult_adc_sin(.a(correct_adc_reg), .b(reg2_sin), .m(adc_sin));
	signed_mult #(.WIDTH_A(WIDTH_1), .WIDTH_B(WIDTH_SIN_COS)) mult_adc_cos(.a(correct_adc_reg), .b(reg2_cos), .m(adc_cos));
	
	accumulator #(.WIDTH(WIDTH_3)) accum_Fs(.clk(clk), .en(line_delay1[0]), .reset(line_delay1[1] & line_delay2), .a(adc_sin), .s(Fs));
	accumulator #(.WIDTH(WIDTH_3)) accum_Fc(.clk(clk), .en(line_delay1[0]), .reset(line_delay1[1] & line_delay2), .a(adc_cos), .s(Fc));
	accumulator #(.WIDTH(WIDTH_2)) accum_S(.clk(clk), .en(line_delay1[0]), .reset(line_delay1[1] & line_delay2), .a(sin2), .s(S));
	accumulator #(.WIDTH(WIDTH_2)) accum_C(.clk(clk), .en(line_delay1[0]), .reset(line_delay1[1] & line_delay2), .a(cos2), .s(C));
	accumulator #(.WIDTH(WIDTH_2)) accum_d(.clk(clk), .en(line_delay1[0]), .reset(line_delay1[1] & line_delay2), .a(sin_cos), .s(d));
	...
	signed_mult #(.WIDTH_A(WIDTH_4), .WIDTH_B(WIDTH_5)) mult_Fs_d(.a(Fs), .b(d), .m(Fs_d));
	signed_mult #(.WIDTH_A(WIDTH_4), .WIDTH_B(WIDTH_5)) mult_Fc_s(.a(Fc), .b(S), .m(Fc_s));
	assign A = Fc_s - Fs_d;
	
	signed_mult #(.WIDTH_A(WIDTH_4), .WIDTH_B(WIDTH_5)) mult_Fc_d(.a(Fc), .b(d), .m(Fc_d));
	signed_mult #(.WIDTH_A(WIDTH_4), .WIDTH_B(WIDTH_5)) mult_Fs_c(.a(Fs), .b(C), .m(Fs_c));
	assign B = Fs_c - Fc_d;
	
	signed_mult #(.WIDTH_A(WIDTH_5), .WIDTH_B(WIDTH_5)) mult_C_S(.a(C), .b(S), .m(C_S));
	signed_mult #(.WIDTH_A(WIDTH_5), .WIDTH_B(WIDTH_5)) mult_d_d(.a(d), .b(d), .m(d2));
	//assign delta = C_S - d2;
	
	wire [WIDTH_6-1:0] A_r, B_r; 
	//wire [87:0] delta_r;
	registor #(.WIDTH(WIDTH_6)) A_reg(.clk(clk), .en(line_delay1[1] & line_delay2), .d(A), .q(A_r));
	registor #(.WIDTH(WIDTH_6)) B_reg(.clk(clk), .en(line_delay1[1] & line_delay2), .d(B), .q(B_r));
	//registor #(.WIDTH(88)) delta_reg(.clk(clk), .en(line_delay1[2] & line_delay2[1]), .d(delta), .q(delta_r));
	//---------------------------------------------
	
	//-----------------AMP-------------------------
	//parameter WIDTH_8 = WIDTH_6 + WIDTH_6; // 71 + 71 = 142
	//parameter WIDTH_9 = WIDTH_8 / 2; // 142 / 2 = 71 
	wire [WIDTH_8-1:0] A_r2, B_r2;
	wire [WIDTH_8-1:0] A_r2_add_B_r2;
	wire [WIDTH_9-1:0] sqrt_AB;
	signed_mult #(.WIDTH_A(WIDTH_6), .WIDTH_B(WIDTH_6)) mult_Ar_Ar(.a(A_r), .b(A_r), .m(A_r2));
	signed_mult #(.WIDTH_A(WIDTH_6), .WIDTH_B(WIDTH_6)) mult_Br_Br(.a(B_r), .b(B_r), .m(B_r2));
	...
	assign amp = temp[WIDTH_DATA-1:0]; 
